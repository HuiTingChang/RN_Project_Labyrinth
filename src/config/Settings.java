...
public class Settings {
	...
	public static void reload(String path) {
		...
		MAX_NAME_LENGTH = Integer.parseInt(properties.get("MAX_NAME_LENGTH")
				.toString());
		DEFAULT_PLAYERS = Integer.parseInt(properties.get("DEFAULT_PLAYERS")
				.toString());
		IMAGEFILEEXTENSION = properties.getProperty("IMAGEFILEEXTENSION").toString();
		IMAGEPATH = properties.getProperty("IMAGEPATH").toString();
		LOCALE = new Locale(properties.get("LOCALE").toString());
		LOGINTIMEOUT = Integer.parseInt(properties.get("LOGINTIMEOUT").toString());
		LOGINTRIES = Integer.parseInt(properties.get("LOGINTRIES").toString());
		MOVEDELAY = Integer.parseInt(properties.get("MOVEDELAY").toString());
		MOVETRIES = Integer.parseInt(properties.get("MOVETRIES").toString());
		PORT = Integer.parseInt(properties.get("PORT").toString());
		SENDTIMEOUT = Integer.parseInt(properties.get("SENDTIMEOUT").toString());//.split(" * ")[0])*Integer.parseInt(prop.get("SENDTIMEOUT").toString().split(" * ")[1])*Integer.parseInt(prop.get("SENDTIMEOUT").toString().split(" * ")[2]); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$ //$NON-NLS-6$
		SHIFTDELAY = Integer.parseInt(properties.get("SHIFTDELAY").toString());
		TESTBOARD = Boolean.parseBoolean(properties.get("TESTBOARD").toString()); //$NON-NLS-1$
		TESTBOARD_SEED = Integer
				.parseInt(properties.get("TESTBOARD_SEED").toString());
		USERINTERFACE = properties.get("USERINTERFACE").toString().equals("BetterUI") ? new BetterUI() : null; //$NON-NLS-1$ //$NON-NLS-2$
		// print();
	}
